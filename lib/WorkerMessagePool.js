...
WorkerMessagePool["prototype"] = {
    ...
    "get":          WorkerMessagePool_get,      // WorkerMessagePool#get():Object
    "getToken":     WorkerMessagePool_getToken, // WorkerMessagePool#getToken():UINT32
    "getWorker":    WorkerMessagePool_getWorker,// WorkerMessagePool#getWorker():WorkerMessage
...
// --- implements ------------------------------------------
function WorkerMessagePool_get() { // @ret Object - { worker: WorkerMessage, token: UINT32 }
                                   // @raise Error("No active thread")
                                   // @desc get next worker (Round-Robin)
    return {
...
function WorkerMessagePool_getToken() { // @ret UINT32
                                        // @desc get corrent token.
    return this._counter & 0xffffffff;
}

function WorkerMessagePool_getWorker() { // @ret WorkerMessage
                                         // @raise Error("No active thread")
                                         // @desc get next worker (Round-Robin)
    ...
    var token = ++this._counter & 0xffffffff;

    return activeThreads[token % activeThreads.length];
}
