...
function WorkerMessage(workerScript,  // @arg URLString = "" - new Workers(workerScript)
                       handleMessage, // @arg Function - handleMessage(message:Any, token:String, event:Event):void
...
WorkerMessage["prototype"]["post"]    = WorkerMessage_post;    // WorkerMessage#post(message:Any, transfer:TransferableObjectArray = null, token:String = "", postback:Function = null):void
...
if (_runOnBrowser) {
    ...
    WorkerMessage["prototype"]["newToken"] = WorkerMessage_newToken; // WorkerMessage#newToken(prefix:String = ""):String
...
function WorkerMessage_post(message,    // @arg Any
                            ...
                            token,      // @arg String = "" - postback token
...
//{@dev
    ...
    $valid($type(token,    "String|omit"),  WorkerMessage, "token");
...
//}@dev
    ...
    if (transfer && allow) {
        port["postMessage"]({ "token": token || "", "msg": message }, transfer);
    } else {
        port["postMessage"]({ "token": token || "", "msg": message });
    }
}
...
function WorkerMessage_newToken(prefix) { // @arg String = ""
                                          // @ret String
    return (prefix || "") + (++this._tokenCounter);
}
